...
```text
input: 
จำนวนร้าน และจำนวนช่วงเวลาที่ต้องการตรวจสอบในบรรทัดแรก บรรทัดถัดมารับเวลาเปิดและปิดของร้านตาม input จำนวนร้านที่ใส่ และบรรทัดสุดท้าย รับช่วงเวลาที่ต้องการตรวจสอบ โดยทุกอย่างที่เป็นเวลาเป็นการคำนวณเป็นหลักนาทีใน 1 วันหมด

output: 
แสดงผลลัพธ์ที่เป็นจำนวนร้านที่เปิดอยู่ในเวลานั้นๆ ที่เรากำหนดในบรรทัดสุดท้าย

Constraints:
จำนวนร้านค้า และเวลาที่มากทำให้ติด run time error ได้ และเงื่อนไขที่ เวลาปิดร้านจะไม่ถูกคำนวณโดยเด็ดขาด ( เปิด <= ทำการ < ปิด) รวมถึงตรวจสอบได้ภายในวันเดียว 1440 นาทีเท่านั้น
...
```text
step1: รับค่าตามโจทย์
step2: ใช้ if else เช็คตามตัวแปรที่สร้างไว้เลย

วิธีนี้ไม่เวิร์คเพราะ output ไม่ตรงกับโจทย์ และฉันยังสับสนในตัวโจทย์เองด้วย ฉันเลยไปถามเพื่อน และ AI
```
...
```text
วิธีสุดท้ายของฉันใช้ list ในการตรวจสอบ โดยการสร้าง list ทั้งหมด 1441 ตัว มีค่าเป็น 0 และใช้ for loop รับค่าตอนเปิด และปิดของแต่ละร้าน จากนั้นใช้ for loop อีกรอบเพื่อเพิ่มเวลาที่ร้านเปิดทำการ (นับเป็นนาทีลงใน list โดยที่เพิ่มจากการถ้าร้านเปิดในเวลานี้ list จะมีการ +1 ลงไป เพื่อแสดงให้เห็นว่าร้านยังเปิดทำการอยู่) จากนั้นสร้าง input ตัวสุดท้ายเพื่อเช็คเวลาที่อยากตรวจสอบ 

และ for loop ตัวสุดท้าย เพื่อนำ แต่ละเวลาที่เช็คไปหา index ใน list เวลาที่เราเพิ่มไปก่อนหน้านี้
```
...
```text
ทดสอบเงื่อนไขที่ใกล้กับเวลาปิดร้าน ( 0 ≤ k ≤ 1440 )
```
...
```text
เช็คเงื่อนไขที่ไม่มีร้านเปิดเลย
```
...
```text
ช่วยไกด์วิธีการ ด้วยการแนะนำวิธีที่ใช้ list เป็นสล็อตเพื่อนับช่วงเวลาที่ร้านเปิดยันปิด
```

คุณยังทำอะไรด้วยตนเอง

```text
แก้ไข ปรับปรุงโค้ด และทดสอบโค้ดด้วยตนเอง
```
