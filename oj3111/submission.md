...
```text
input: รับค่า 3 ตัว โดยที่ 1) สมาชิกสหกรณ์ 2)จำนวนรายการสินค้าที่ซื้อ 3)ราคาสินค้าทั้งหมด

output: ต้องจ่ายกี่บาท

Constraints: การปัดเศษแบบปัดเศษมาตรฐาน – round half up เพราะฟังก์ชันบางตัว ไม่ได้ปัดเศษแบบนี้
```
...
```text
step1: รับค่าเป็น string
step2: รับค่าจำนวนสินค้าที่ซื้อทั้งหมด
step3: สร้างตัวแปลที่นับรวมจำนวนที่ต้องจ่ายทั้งหมด
step4: ใช้ for loop รับค่าตามจำนวนสินค้าที่ซื้อ
step5: ตรวจสอบเงื่อนไขว่า เป็นสมาชิกสหกรณ์รึเปล่า ถ้าเป็น ได้ลด 5%
step6: ถ้าไม่ได้เป็นแต่ซื้อของเกินหรือเท่ากับ 500 ได้ลด 3% 
step7: แต่ถ้าไม่ตรงกับเงื่อนไขใดจ่ายราคาเต็ม

ก่อนถามเพื่อนฉันได้ลองใช้ math.ceil ในการปัดเศษ แต่เหมือนว่าจะยังผิดบาง testcase
```
...
```text
วิธีสุดท้ายที่ฉันใช้ ฉันใช้แก้ไขการปัดเศษของฉันด้วยใช้ math.ceil แต่ปัดเป็นทศนิยม 2 ตัวแหน่งด้วยการนำผลลลัพธ์ไปคูณ 100 แล้วหารด้วย 100 จากนั้น ใช้ f-string ในการปัดเลข 2 ตัว เพื่อกันกรณีที่ผลลัพธ์เป็น .00
```
...
```text
ถ้าผลลัพธ์เป็น .999 ควรจะปัดไปเป็นอีกค่านึงที่ได้ .00
```
...
```text
เช็คว่าถ้าผลลัพธ์ได้ .0 ควรออกมาเป็น .00
```
...
```text
ทดสอบเงื่อนไขที่ซื้อเกิน 500 ได้ลด 3%
```
...
```text
ให้คำแนะนำเรื่องการปัดเศษ
```

คุณยังทำอะไรด้วยตนเอง

```text
แก้ไขโค้ด ทดสอบ testcase
```
