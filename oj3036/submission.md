...
```text
input:
 เลขห้องในปราสาทเป็น int

output:
วิธีการที่เร็วที่สุดที่จะถึงยอดปราสาท โดยนับว่าผ่านกี่กำแพง

Constraints:
พืจารณาว่าแต่ละห้องมีกำแพงไม่เท่ากัน บางห้องมี 2 บางห้องมี 1
```
...
```text
step 1: รับค่าหมายเลขห้อง
step 2: สร้างตัวแปรสำหรับเก็บชั้น โดยเริ่มจากชั้นที่ 1 (หัวปราสาท)
step 3: ใช้ for loop ในการหากำแพงของแต่ละห้อง

ฉันเขียนต่อไม่ได้เลยไปขอคำแนะนำจากเพื่อน 
```
...
```text
วิธีสุดท้่ายหลังขอคำแนะนำจากเพื่อน ฉันใช้ while loop ในการหาว่า ห้องที่ป้อนลงไปอยู่ชั้นไหนในปราสาทนี้ โดยที่ปราสาทนี้จะมีเลขห้องทั้งหมด ชั้น**2 (เช่นให้ชั้นเท่ากับ 4 ห้องสุดท้ายของชั้นนั้นก็คือห้อง 16) และจากนั้นสร้างตัวแปร layer ไว้ว่าอยู่ห่างจากยอดกี่ชั้น โดยที่แต่ละชั้นจะมี pattern สอดคล้องกันแบบ (n*2) - 1 และสร้างตัวแปร distance สำหรับหาว่าห้องนั้นเป็นห้องที่เป็นเลขคู่หรือเลขคี่ (ห้องเลขคี่มี 2 กำแพง ในทางกลับกันห้องเลขคู่มี 1 กำแพง) โดยใช้ (ชั้น**2) - ห้อง เพราะหาห้องริมสุดลบด้วยห้องที่เรา input เข้าไป 

หลังจากนี้ใช้ if-else โดยที่เงื่อนไขแรก หากห้องที่ input ไปตอนแรก ถ้าเป็นห้องเลขคู่จะใช้กำแพงเดียว ก็คือ - 1 ห้องเลขคี่ใช้ 2 กำแพง
```
...
```text
ต้องการทราบว่า ถ้าเราอยู่ยอดปราสาทแล้วโปรแกรมยังทำงานถูกมั้ย
```
...
```text
ถ้าเป็นเลขคู่ ต้องออก output มาน้อยกว่าลขคี่ 1 ตัว ในที่นี้ใช้ 22 ซึ่งคล้ายกับ sample testcase
```
...
```text
ทดสอบว่าถ้าเป็นห้องริมปราสาทจะทำงานได้มั้ย
```
...
```text
ช่วยอธิบายวิธีการคิดข้อนี้
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ด ทดสอบ testcase แก้บัค และส่ง OJ
```
