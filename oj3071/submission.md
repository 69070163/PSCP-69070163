...
```text
input :
โปรแกรมบรรทัดแรกรับค่าชวงตัวเลขเริ่มต้น A บรรทัดถัดมารับค่าช่วงตัวเลขตัวสุดท้าย B บรรถัดที่ 3 รับค่า ตัวเลข d ที่หาร  และบรรทัดสุดท้ายรับเศษ (r) ที่ได้จากการหารบรรทัดที่ 3

output :
แสดงผลรวมว่า ระหว่าง A และ B มีตัวที่หารด้วย d ได้ เศษ r กี่ตัว

Constraints:
พอดูเป็นช่วงก็ควรใช้ for loop เพื่อรันเลขตั้งแต่ A ถึง B รวมถึงใช้ B + 1 เพราะ Python รันเลขที่ 0 และใช้ modulo เพื่อหาว่าเศษเหลือเท่ากับ r มั้ย
...
```text
step1: รับค่า A B d r
step2: สร้างตัวแปร total เพื่อเก็บจำนวนครั้งที่ ระหว่าง A ถึง B หารด้วย d ได้เศษ r กี่ตัว
step3: ใช้ for loop เพื่อรันเลขตั้งแต่ A ถึง B 
step 4: นำ i ไปหารเอาเศษกับ d  และเช็คว่า ผลลัพธ์เท่ากับ r หรือไม่
step 5: ถ้าเท่า บวก1 ลง total
```
...
```text
วิธีสุดท้ายของฉันเหมือนแผนแรก ฉันใช้ modulo ตรวจสอบว่า จำนวนระหว่าง A ถึง B หารด้วย d ลงตัวแล้วเท่ากับ r มั้ย ถ้าเท่าก็บวก 1 ลง total ถ้าไม่เท่าก็เช็คถัดไป และใน renge ของ for loop ฉันก็เพิ่ม +1 ลงใน B เพราะ Python จะรันเลขตั้งแต่ 0 
```
...
```text
ทดสอบเลขน้อยๆ หลักหน่วย ว่าสามารถเช็คค่าได้มั้ย
```
...
```text
ทดสอบว่าถ้า A เป็น 0 กับ B เป็น 1 โค้ดจะต้องออกมาเป็น 0 เพราะ 0 หรือ 1 หารอะไรก็ลงเงื่อนไขที่ total จะบวกเพิ่ม
```
...
```text
ลองเช็คกรณีที่ A ถึง B โดยที่ B เป็นเลขที่ตรงกับเงื่อนไขพอดี เพื่อเช็คการทำงานของ range ใน for loop
```
