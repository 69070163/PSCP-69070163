...
```text
input : จำนวนเงินที่ลูกค้าจ่ายเป็น flaot
output : ผลรวมราคาทั้งหมดหลังรวมค่าบริการ และ vat7%
constraints : 1. การคิดราคาขั้นต่ำของค่าบริการที่ต้องมีขั้นต่ำ 50 บาท แต่ไม่เกิน 100 บาท และต้องคิดแยกกับราคาอาหารที่รวม vat7%
```
...
```text
1. รับ input ราคาค่าอาหารมา เป็นตัวแปรชนิด float 
2. คำนวนหาค่าบริการจากที่โจทย์กำหนดไว้คือ 10%ของ input
3. ใช้ if-else มาเช็ค condition ในกรณีที่ค่าบริการถูกกว่า 50 ก็จะบังคับให้เป็น 50 บาท แต่ถ้าเกิน 1000 ก็จะบังคับเป็น 1000
4. คำนวนหา vat7% หลังจากรวมค่าบริการกับ input แล้ว
5.นำทุกเลขมาบวกกัน และแสดงผล output 
...
```text
ในขั้นตอนสุดท้ายลองเพิ่มกรณีที่ลูกค้าไม่ได้ซื้ออะไรเลย (input = 0) ขึ้นมา โดยเพิ่มเป็นเงื่อนไขแรกในการเช็คราคาค่าบริการ ก่อนนำไปคิดในขั้นตอนต่อไป
```
...
```text
เพราะยังไงต้องการตรวจสอบว่าถ้าราคาอาหารถูกกว่าค่าบริการจะยังมีการคิดค่าบริการขั้นต่ำอยู่มั้ย
```
...
```text
เพราะยังไงต้องการตรวจสอบว่าถ้าราคาอาหารเท่ากับค่าบริการขั้นต่ำ จะยังมีการคิดค่าบริการเป็นแบบไหน
```
...
```text
เพราะว่าถ้าลูกค้าไม่ซื้ออะไรเลย (input = 0) โค้ดยังจะต้องสามารถรันต่อได้
```
...
```text
ไม่ได้ใช้ความช่วยเหลือจากคนและ AI
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ด และทดสอบ testcase
```
