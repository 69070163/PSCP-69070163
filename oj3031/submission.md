...
```text
input : 
อินพุตสองตัวโดยตัวแรกคือพื้นที่น้ำหมึกขยายต่อ 1 วินาที ตัวถัดมาคือจำนวนคำถามของผู้คน จากนั้นรับ input พิกัดบ้านของผู้คนที่ส่งคำขอมา

output :
อีกกี่วินาทีที่บ้านของคนนั้นจะโดนน้ำหมึกท่วม โดยปัดเศษ

constraints :
ควรพิจารณาสูตรการหาพื้นที่วงกลมให้ดี ระวังเรื่องการปัดเศษ
```
...
```text
step 1: รับค่า input โดยใช้ฟังก์ชัน map ในการทำให้ค่าที่รับมาเป็น int หมด และใช้ฟังก์ชัน split() ในการรั[ input มากกว่า 1 ตัว โดยให้ input ตัวแรกเป็น main_area สำหรับพื้นที่น้ำหมึกขยาย/วินาที และ request สำหรับจำนวนคำขอของผู้คน
step 2: รับค่า x, y  ของคำขอผู้คน โดยใช้ for loop 
step 3: หาค่า r ของคำขอผู้คน
step 4: นำค่า r มาเข้าสูตรหาพื้นที่
step 5: นำค่าพื้นที่ได้มาหาร main_area

หลังจากนี้ฉันไปขอให้เพื่อนไกด์ให้ เพราะลองรัน testcase แล้วผิด
```
...
```text
วิธีสุดท้ายที่ฉันใช้จริง หลังจากที่เพื่อนแนะนำ ฉันลองเปลี่ยนสูตรจากการหา r ตรงๆ ไปเป็นการใช้สูตร  3.1416 * (x*x) + (y*y) ซึ่งเป็นสูตรการหาพื้นที่วงกลมจากพิกัด x/y และลองใช้ math.ceil ในการปัดเศษ
```
...
```text
ทดสอบว่า ถ้าจำนวนหมึกที่หยดมาเป็น 0 แล้วจะเกิดอะไรขึ้น
```
...
```text
ให้เขาช่วยไกด์การทำโจทย์ข้อนี้ พร้อมทั้งอธิบายวิธีการ รวมถึงให้แนวทางการใช้สูตร
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดหลังจากเพื่อนแนะนำ รวมถึงแก้ไขบัค และทดสอบ testcase ด้วยตนเอง
```
