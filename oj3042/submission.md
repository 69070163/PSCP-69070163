...
```text
input : 
ใส่เลขจำนวนเต็มบวก 1 ตัว

output : 
แสดงจำนวนตัวเลขทุกตัวที่หารสิบลงตัว

constraints : 
ใน output เป็นเลขนับจากเลขที่มากที่สุดไปน้อยที่สุด ฉันควรจะใช้ตัว (start,stop,step) ให้ดี
```
...
```text
step 1: รับค่าจำนวนเต็ม num
step 2: ใช้ for loop ในการเข้าถึงจำนวน num ทุกตัว
step 3: ตั้งเงื่อนไขถ้า i หาร10 แล้วเท่ากับ 0 (ในที่นี้ใช้ %) ถ้าจริงให้ num เก็บค่า i
step 4: แสดง output ค่า num ใช้คำสั่ง end=" " เพื่อให้ num แสดงผลเป็นแถวเดียว ไม่ขึ้นบรรทัดใหม่
```
...
```text
วิธีสุดท้ายเหมือนแผนแรกของฉัน ใช้ modulo เพื่อตรวจสอบว่า i หารด้วย 10 แล้วเท่ากับ 0 ใช่หรือไม่ โดยที่ i คือค่าตั้ง num ถึง 0 แต่ฉันใช้ num ใน start และใช้ -1 ใน stop เพราะว่า python นับ index ที่ 0 รวมถึงใช้ -1 ใน step เพราะต้องการให้ num รันเลขถอยหลังจากมากไปน้อย
```
...
```text
เพราะถ้าเกิด num เป็นเลขหลักหน่วย output จะต้องได้เท่ากับ 0 เพราะมี 0 ตัวเดียวที่หารลงตัว
```
...
```text
เพราะถ้า num เป็น 0 ก็ควรจะมีตัวเดียวที่ 10 หารลงตัว นั่นก็คือ 0
```
...
```text
เพราะต้องการทดสอบกรณีเลข 2 หลักที่มีหลักหน่วยว่าหารลงตัวถูกต้องมั้ย
```
...
```text
ไกด์แนวทางและขั้นตอนของ logic ในการเขียนโค้ด
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันเขียนโค้ดเองทั้งหมด โดยการรับฟังคำแนะนำจากเพื่อนและนำมาเขียนในโค้ดของตนเอง
```
