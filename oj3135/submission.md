...
```text
input:
จำนวนผู้เล่นแทนด้วย n ต้องส่งของขวัญไปให้ในตำแหน่ง + k และตำแหน่งที่โจรนั่งแทนด้วย t

output:
แสดงจำนวนคนที่ได้จับของขวัญ ถ้าคนแรกเป็นโจรเลยก็นับ

Constraints:
การวนเช็คของขวัญเป็นวงกลม และถ้าเกิดของขวัญวันกลับไปถึงมือคนแรก หรือโจรก็นับว่าจบ
```
...
```text
step1 : รับค่า n k t บรรทัดเดียวกัน
step2 : สร้างตัวแปร times = 0 gift = 1 (แทนตำแหน่งคนที่ 1)
step3 : ใช้ for loop ในการ loop ค่าวงกลม
step4 : ให้เงื่อนไขว่าถ้า t = 1 หรือในกรณีที่คนแรกที่จับของขวัญเป็นโจร เกมจบทันที
step5 : นอกนั้น บวกเพิ่มตำแหน่งไปเรื่อยๆ แต่ถ้าไม่เกิน n ก็นับ +1 ถ้าเกิน ลบออกด้วยจำนวน n และเขียนเงื่อนไขข้างในต่อว่า gift ไม่เท่ากับ 1 หรือ ไม่เท่ากับ t ก็จะบวก 1 และไม่ออกจากลูป แต่ถ้าไม่ใช่ จะออกจากลูป
```
...
```text
ฉันเปลี่ยนจากการใช้ for loop เป็น while loop และเริ่มจากการเช็คว่า ตำแแหน่งที่ 1 เท่ากับตำแหน่งของโจรมั้ย ถ้าใช่ ก็ออกจาก loop แต่ถ้าไม่ใช่ ให้คำนวนตำแหน่งจากการ current = ((current - 1) + k) % n + 1  คือตำแหน่ง บวกด้วย ระยะห่าง (k) ที่กำหนดไว้ มา modulo ด้วย n ในวงเล็บแรกและวงเล็บหลังมี - 1 , +1 สำหรับการคำนวน index ในคอมพิวเตอร์

จากนั้นแยกกรณี โดยที่ถ้าตำแหน่ง = 1 หรือเท่ากับโจรออกจาก loop ได้เลย แต่ถ้ายังไม่ออกจาก loop ก็ +1 
```
...
```text
ทดสอบว่า ถ้าโจรคือคนที่ 1 โค้ดจะทำงานถูกต้องมั้ย
```
...
```text
ให้คำแนะนำในขั้นตอนแรก
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันคิดว่าแผนของเพื่อนไม่เวิร์คสำหรับวิธีที่ฉันทำ ฉันเลยลองปรับแพลนด้วยตนเอง
```
