...
```text
Input:
ต้นทาง ปลายทางที่ส่งพัสดุ รับค่าเป็น string input และบรรทัดถัดมาเป็นน้ำหนักพัสดุ

Output:
ต้องจ่ายเงินค่าจัดส่งพัสดุเท่าไหร่

Constraints:
แต่ละเงื่อนไขมีค่าธรรมเนียมเริ่มต้น และน้ำหนักไม่เท่ากัน ควรอ่านโจทย์ดีๆ ป้องกันการสับสน
```
...
```text
step 1: รับค่าต้นสาย ปลายทาง 2 ค่าในบรรทัดเดียวกัน บรรถัดถัดมารับค่าน้ำหนัก
step 2: เช็คว่าต้นสายปลายทางคืออันไหน
step 3: คำนวนออกมาว่าต้องจ่ายกี่บาท ตามเงื่อไขของต้นสายปลายทาง
```
...
```text
วิธีสุดท้ายที่ฉันใช้เหมือนกับวิธีแรกเลย
```
...
```text
ทดสอบว่าสามารถรันให้ output เป็น error ได้หรือไม่ ด้วย output นอกเหนือจากที่กำหนด
```
...
```text
เช็คเงื่อนไขที่ไม่อยู่ใน sample testcase
```
