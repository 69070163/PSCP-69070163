...
```text
input: 
หวยที่กระต่ายน้อยซื้อ และหวยที่ถูกรางวัลที่ 1

output:
เงินรางวัลที่กระต่ายน้อยได้ หรือถ้ากระต่ายน้อยซื้อไม่ถูกสักตัว กระต่ายน้อยก็ได้ 0 บาท

Constraints:
การเรียงลำดับเงื่อนไข
```
...
```text
step1 : รับค่า input และใช้ spilt ในการรับค่า ตัวอักษรกับเลขแยก
step2 : สร้างอีกตัวแปรนึง โดยที่ยึด index ที่ 1 ที่เป็น list ของค่าที่รับมาก่อนหน้านี้
step3 : เข้าเงื่อนไข โดยเริ่มจากเงื่อนไขรางวัลที่ 1 ที่ทั้งเลขทั้งตัวอักษรตรงกันหมด กระต่ายน้อยได้ล้านบาท
step4 : ค่อยๆไล่เงื่อนไขตามโจทย์จนครบ โดยการเช็คตัวเลขด้วย sting slicing
```
...
```text
วิธีสุดท้ายของฉันลองสลับเงื่อนไข เนื่องจากรันไม่ผ่าน oj โดยฉันแก้ไขด้วยการนำเงื่อนไขที่กระต่ายน้อยได้ 2000 บาทขึ้นก่อนแล้วค่อยตามด้วย 1000 เพื่อให้โค้ดเช้คจากเงื่อนไขนี้ก่อนแล้วค่อยเช็คเงื่อนไขถัดไป
```
...
```text
ชี้ให้เห็นว่าการวางเงื่อนไขของฉัน ฉันเลยติด testcase
```
